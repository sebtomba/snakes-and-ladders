...
class EventLogger extends Actor {
  ...
  def receive = {
    ...
    case PlayerRolledDice(player, rolled) => log.info("{} rolled a {}", player.name, rolled)
    ...
    case PlayerMovedToPosition(position) => log.info("{} moved to field {}", position.player.name, position.position)
    case PlayerMovedBySnake(position) => log.info("{} moved by snake to field {}", position.player.name, position.position)
    case PlayerMovedByLadder(position) => log.info("{} moved by ladder to field {}", position.player.name, position.position)
