...
class GameStateMachine extends FSM[GameState.State, Data] {
  ...
  when(DeterminingPlayOrder) {
    ...
    case Event(PlayerRolled(player, rolled), PlayOrderData(_, playOrder, _))
    ...
    case Event(PlayerRolled(player, rolled), PlayOrderData(game, playOrder, currentPlayer)) =>
  ...
  when(MainGame) {
    ...
    case Event(PlayerRolled(player, rolled), data @ GameData(game, playOrder, positions, currentPlayer, _))
